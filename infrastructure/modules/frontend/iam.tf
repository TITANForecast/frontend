...
# Custom policy for ECS execution role to access SSM parameters and Secrets Manager
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_secretsmanager_secret.frontend_config.arn
        ]
      }
    ]
  })
...
# Custom policy for ECS task role (if needed for application-specific permissions)
