...
/**
 * GET /api/dealer-settings/ro/[serviceRecordId]/pdf
 * Generate PDF for a service record (RO)
 */
export async function GET(
  ...
  try {
    ...
    const fs = financialSummaries && financialSummaries.length > 0 ? financialSummaries[0] : null;
    ...
    const pdfData: ROPDFData = {
      ...
      vehicle: {
        ...
        mileageIn: sr.ro_mileage ? parseFloat(sr.ro_mileage.toString()) : undefined,
        mileageOut: sr.mileage_out ? parseFloat(sr.mileage_out.toString()) : undefined,
      },
      serviceRecord: {
      ...
      operations: operationsWithDetails.map((op) => ({
        ...
        laborLines: (op.labor_lines || [])
          ...
          .map((l: any) => ({
            ...
            laborBillHours: l.labor_bill_hours
              ...
              : undefined,
            laborSale: l.labor_sale ? parseFloat(l.labor_sale.toString()) : undefined,
            laborCost: l.labor_cost ? parseFloat(l.labor_cost.toString()) : undefined,
            repairIn: undefined,
            repairOut: undefined,
          })),
        partsLines: (op.parts_lines || [])
          ...
          .map((p: any) => ({
            ...
            quantity: p.part_quantity ? parseFloat(p.part_quantity.toString()) : undefined,
            unitCost: p.parts_unit_cost ? parseFloat(p.parts_unit_cost.toString()) : undefined,
            unitSale: p.parts_unit_sale ? parseFloat(p.parts_unit_sale.toString()) : undefined,
    ...
    // Convert PDF to buffer using a promise
    ...
    return new NextResponse(pdfBuffer as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="RO-${sr.ro_number || serviceRecordId}.pdf"`,
      },
    });
  } catch (error) {
