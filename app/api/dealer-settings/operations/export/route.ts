...
// Streaming requires the Node.js runtime and must opt out of static analysis.
// Without these, Next.js standalone bundling produced a server build that
// hung on POST handling for unrelated routes — see issue #67.
 ...
 *
 * Streams data directly from database to client in batches, with minimal
 * memory regardless of dataset size. SECURITY NOTE: this route uses
 * `$queryRawUnsafe` with string interpolation to match the rest of the
 * codebase. Inputs (dealerId, serviceIds, search, payTypes) reach SQL
 * unparameterized — this is a known shared-codebase issue tracked
 * separately and should be migrated to `$queryRaw` template literals
 * along with the other admin/dms/dashboard routes.
 */
